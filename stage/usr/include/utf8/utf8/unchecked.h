...
namespace utf8
{
    namespace unchecked
    {
        ...
        template <typename octet_iterator>
        int32_t utf8toiso8859(octet_iterator start, octet_iterator result, size_t size)
        {
          std::int32_t retSize = 0;
          while (*start) {
            if (size == 0)
              return -1;
            size--;
        ...
        template <typename octet_iterator>
        int32_t iso8859toUtf8(octet_iterator start, octet_iterator result, std::size_t size)
        {
          int32_t retSize = 0;
          uint8_t index;
          while (*start)
          {
            if (size < 2)
              return -1;
            index = static_cast<uint8_t>(*(start++));

            octet_iterator newresult = append(index, result);
            size -= newresult - result;
            retSize += static_cast<uint32_t>(newresult - result);
            result = newresult;
          }
          if (size == 0)
            return -1;
        ...
        template <typename octet_iterator>
        int32_t utf8len(octet_iterator start)
        {
          int32_t k, len = 0;
          while (*start)
          {
            k = utf8::internal::sequence_length(start);
            if (k)
            {
              start += k;
              ++len;
